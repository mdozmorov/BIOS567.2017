...
head(filters)
filters[grep("affy",filters[, "name"]),] # List specific ones
...
genes<-getBM(attributes=c('ucsc','hgnc_symbol','description'), 
             filters='ucsc', values=IDs, mart=mart)#, uniqueRows=T)
# Convert Gene names IDs (hgnc_symbol, mgi_symbol)
genes<-getBM(attributes=c('wikigene_name','hgnc_symbol','description', 'ensembl_gene_id'), 
             filters='hgnc_symbol', values=IDs, mart=mart)#, uniqueRows=T)
# Convert entrezgene
genes<-getBM(attributes=c('hgnc_symbol','description'), filters='entrezgene', values="1", mart=mart, uniqueRows=T)
# Convert RefSEq IDs
genes<-getBM(attributes=c('refseq_mrna','hgnc_symbol','description'), 
